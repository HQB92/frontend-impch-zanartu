...
export const CustomersTable = (props) => {
  ...
  const generarPDF =  (data) => {
    ...
    doc.setFontSize(15);
    doc.addImage(Bautizo.data, 'PNG', 0, 0, 216, 216);
    doc.text(`${data.childFullName}`, 36, 84 );
    doc.text(`${data.fatherFullName}`, 43, 92);
    doc.text(`${data.motherFullName}`, 37, 99);
    doc.text(`${data.placeOfRegistration}`, 57, 106);
    doc.text(`${fechaNacimiento[0]}`, 118, 106);
    doc.text(`${fechaNacimiento[1]}`, 136.5, 106);
    doc.text(`${fechaNacimiento[2]}`, 175, 106);

    doc.text(`${data.childRUT}`, 39, 156);
    doc.text(`${data.registrationNumber}`, 137, 156);
    doc.text(`${fechaRegistro[0]}`, 35, 163.5);
    doc.text(`${fechaRegistro[1]}`, 53, 163.5);
    doc.text(`${fechaRegistro[2]}`, 91, 163.5);
